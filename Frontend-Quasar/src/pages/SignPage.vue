    ...
    <div class="main">
      <div class="container a-container" id="a-container">
        <form class="form" id="a-form">
            ...
            <div style="display:flex; align-items:center;">
              <input class="form__input" type="text" v-model="signupEmail" placeholder="이메일">
              <button class="duplication__box" @click="emailDuplicationCheckAndVerify">인증메일</button>
            </div>
            <div style="display:flex; align-items:center;">
              <input class="form__input" type="text" v-model="signupNickname" placeholder="닉네임">
              <button class="duplication__box" @click="nicknameDuplicationCheck">중복체크</button>
            </div>
            <input class="form__input_pass" type="password" v-model="signupPassword" placeholder="비밀번호">
            <input class="form__input_pass" type="password" v-model="signupCheckPassword" placeholder="비밀번호 확인">
            <button class="form__button button submit" @click="signup">회원가입</button>
        </form>
      </div>
      <div class="container b-container" id="b-container">
        <form class="form" id="b-form">
            ...
            <input class="form__input" type="text" v-model="loginEmail" placeholder="이메일">
            <input class="form__input" type="password" v-model="loginPassword" placeholder="비밀번호">
            <a class="form__link">비밀번호를 잊으셨나요?</a>
            <button class="form__button button submit" @click="login">로그인</button>
        </form>
      </div>
      <div class="switch" id="switch-cnt">
        ...
        <div class="switch__container" id="switch-c1">
          ...
          <p class="switch__description description">저희와 함께하기 위해 로그인하러 가시죠!</p>
          <button class="switch__button button switch-btn" @click="toggle">로그인하러 가기</button>
        </div>
        <div class="switch__container is-hidden" id="switch-c2">
          <h2 class="switch__title title">어서오세요!</h2>
          <p class="switch__description description">어서 개인정보를 탈취할 수 있게 알려주세요!</p>
          <button class="switch__button button switch-btn" @click="toggle">회원가입하러 가기</button>
        </div>
      </div>
    </div>
  </q-page>
...
export default defineComponent({
  ...
  methods: {
    ...
    emailDuplicationCheckAndVerify () {
      const email = this.signupEmail
      this.$store.dispatch('emailDuplicationCheck', email)
      this.$store.dispatch('emailVerification', email)
    },
    nicknameDuplicationCheck () {
      const nickname = this.signupNickname
      this.$store.dispatch('nicknameDuplicationCheck', nickname)
    },
    signup () {
      if (this.signupEmail === '' || this.signupPassword === '' || this.signupNickname === '') {
        alert('모든 내용을 입력해주세요')
        return
      }
